...
import argparse
from model_util import DataPrepare, LSTNetInit, get_arguments
from model import PreSkipTrans, PostSkipTrans, PreARTrans, PostARTrans, LSTNetModel, ModelCompile
from plots import AutoCorrelationPlot, PlotHistory, PlotPrediction
...
if __name__ == "__main__":
    ...
    # Reading data
    Data = DataPrepare(lstnet_init.data,
    ...
    if lstnet_init.train is True:
        ...
        if lstnet_init.plot is True:
            PlotHistory(h.history, ['loss', 'rse', 'mean_absolute_error'], lstnet_init)
        ...
        if lstnet_init.saveresults == True:
            SaveResults(lstnet, lstnet_init, h.history, test_result, ['loss', 'rse', 'mean_absolute_error'])
    ...
    if lstnet_init.train is False and lstnet_init.validate is True:
        loss, rse, mae = lstnet.evaluate(Data.valid[0], Data.valid[1])
        log.info("Validation on the validation set returned: Loss:%f, RSE:%f, MAE:%f", loss, rse, mae)
    elif lstnet_init.validate == True:
        log.info("Validation on the validation set returned: Loss:%f, RSE:%f, MAE:%f",
                 h.history['val_loss'][-1], h.history['val_rse'][-1], h.history['val_mean_absolute_error'][-1])

    # Testing evaluation
    if lstnet_init.evaltest is True:
        loss, rse, mae = lstnet.evaluate(Data.test[0], Data.test[1])
        log.info("Validation on the test set returned: Loss:%f, RSE:%f, MAE:%f", loss, rse, mae)
        test_result = {'loss': loss, 'rse': rse, 'mae': mae}
