...
def LSTNetModel(init, input_shape):
    ...
    # R = Bidirectional(LSTM(init.GRUUnits, activation='selu', return_sequences=False, return_state=False, kernel_initializer=init.initialiser))(R)
...
def ModelCompile(model, init):
    ...
    if init.optimiser == "SGD":
        opt = SGD(lr=init.lr, momentum=0.9, decay=1e-5, nesterov=False)
    ...
    model.compile(optimizer = opt, loss = init.loss, metrics=[rse, 'mean_absolute_error'])
